...
class NotebookKernel:
    ...
    async def run_(
        ...
        comm_open_cb: "Optional[Callable[[], Any]]" = None,
        comm_msg_cb: "Optional[Callable[[], Any]]" = None,
        comm_close_cb: "Optional[Callable[[], Any]]" = None,
    ) -> "None":
        ...
        async def process_execute_iopub_rsp() -> "None":
            ...
            async for rsp in self.await_iopub_rsps(msg_id):
                ...
                if msg_type == "status":
                    ...
                    if callable(comm_open_cb):
                        comm_open_cb(rsp.get("content", {}))

                elif msg_type == "comm_msg":
                    if callable(comm_msg_cb):
                        comm_msg_cb(rsp.get("content", {}))

                elif msg_type == "comm_close":
                    if callable(comm_close_cb):
                        comm_close_cb(rsp.get("content", {}))
        ...
        if self.kc is None:
            ...
            self._aodo(
                self.run_(
                    ...
                    output_cb=output_cb,
                    done_cb=done_cb,
                    comm_open_cb=comm_open_cb,
                    comm_msg_cb=comm_msg_cb,
                    comm_close_cb=comm_close_cb,
                ),
                wait=wait,
            )
