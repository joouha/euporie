...
def _separate_buffers(
    substate: "Union[List, Dict, Tuple]",
...
class IpyWidgetComm(Comm, metaclass=ABCMeta):
    ...
    def _get_embed_state(self) -> "Dict[str, Any]":
        ...
        return {
            "model_name": self.data["state"]["_model_name"],
            "model_module": self.data["state"]["_model_module"],
            "model_module_version": self.data["state"]["_model_module_version"],
            "state": {
                **{
                    key: value
...
class LayoutIpyWidgetComm(IpyWidgetComm, metaclass=ABCMeta):
    ...
    def render_children(
        self, models: "List[str]", cell: "Cell"
    ) -> "List[AnyContainer]":
...
class ButtonModel(IpyWidgetComm):
    ...
    def text(self) -> "str":
        ...
        if icon := self.data["state"].get("icon", ""):
            from euporie.reference import FA_ICONS
...
class ToggleButtonsModel(IpyWidgetComm):
    ...
    def get_label(self, index: "int") -> "AnyFormattedText":
        ...
        label = str(self.data["state"]["_options_labels"][index])
...
class ColorPickerModel(TextBoxIpyWidgetComm):
    ...
    def format_color(self) -> "str":
        ...
        from euporie.reference import NAMED_COLORS
