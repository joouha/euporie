"""Renders the changed parts of the terminal output."""

from typing import TYPE_CHECKING

from prompt_toolkit import renderer as ptk_renderer
...
def _output_screen_diff(
    ...
    def get_max_column_index(row: "Dict[int, Char]") -> "int":
        """Get the maximum used column index, ignoring trailing whitespace."""
        numbers = [
            index
...
def patch_renderer() -> "None":
    """Patch the screen diff function used by prompt_toolkit's renderer."""
    ptk_renderer._output_screen_diff = _output_screen_diff
