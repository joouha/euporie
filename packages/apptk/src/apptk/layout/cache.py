...
from apptk.layout.layout import walk
from apptk.layout.mouse_handlers import MouseEvent, MouseHandlers
...
class CachedContainer(Container):
    ...
    def __init__(
        ...
        self._width_cache: FastDictCache[tuple[int, int], Dimension] = FastDictCache(
            get_value=lambda _render_count, max_available_width: (
                self.container.preferred_width(max_available_width)
            )
        )
        self._height_cache: FastDictCache[tuple[int, int, int], Dimension] = (
            FastDictCache(
                get_value=lambda _render_count, width, max_available_height: (
                    self.container.preferred_height(width, max_available_height)
                )
            )
        )
    ...
    def write_to_screen(
        ...
        Args:
            screen: The :class:`~apptk.layout.screen.Screen` class to which
                the output has to be written.
            mouse_handlers: :class:`apptk.layout.mouse_handlers.MouseHandlers`.
            write_position: A :class:`apptk.layout.screen.WritePosition` object
            ...
            parent_style: Style string to pass to the :class:`.Window` object. This will
                ...
                :class:`apptk.layout.containers.HSplit` can use it to pass
