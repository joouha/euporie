...
class Config:
    ...
    def load(self) -> None:
        ...
        with BufferedLogs(logger=log):
            try:
                ...
                self._values.update(
                    self._validate(self._load_user(prefix=app_name), "app config file")
                )
                self._values.update(
                ...
                self._values.update(
                    self._validate(
                        self._load_env(prefix=app_name),
    ...
    def _save(self, setting: Setting) -> None:
        ...
        json_data.setdefault(self._values["app"], {})[setting.name] = self._values[
            setting.name
        ]
        if self._valid_user:
    ...
    def _load_args(self) -> dict[str, Any]:
        ...
        self._populate_parser(self.parser)
        # Parse known arguments
        namespace, _remainder = self.parser.parse_known_intermixed_args()
        # Validate arguments
